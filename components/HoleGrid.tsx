...
function getHoleStyle(score: ScoreType | null): { bg: string; text: string; border: string } {
  switch (score) {
    case 'eagle':
      return { bg: '#134d2a', text: '#ffffff', border: '#0f3d1f' }
    case 'birdie':
      return { bg: '#1D6B3B', text: '#ffffff', border: '#155c30' }
    case 'par':
      return { bg: '#dcfce7', text: '#1D6B3B', border: '#86efac' }
    default:
...
function getScoreIcon(score: ScoreType | null, color: string) {
  if (score === 'eagle') return <span className="text-xs mt-0.5 leading-none">🦅</span>
...
export default function HoleGrid({ holeDetails, scores, onHoleTap, celebratingHole }: HoleGridProps) {
  ...
  return (
    <div className="grid grid-cols-6 gap-2.5 p-4">
      {holes.map((holeNum) => {
        ...
        const style = getHoleStyle(bestScore)
          ...
          <button
            ...
            style={{
              ...
              boxShadow: bestScore === 'eagle' || bestScore === 'birdie'
                ? '0 4px 12px rgba(29,107,59,0.25)'
